...
namespace Newtonsoft.Json
{
	...
	public class JsonSerializer
	{
		...
		/// <param name="value">The <see cref="Object"/> to serialize.</param>
        public void Serialize(JsonWriter jsonWriter, object value, bool isIgnoreCollectionProperty)
		{
			if (jsonWriter == null)
            ...
            SerializeValue(jsonWriter, value, isIgnoreCollectionProperty);
		}

        public void Serialize(JsonWriter jsonWriter, object value)
        {
            Serialize(jsonWriter, value, false);
        }

        private void SerializeValue(JsonWriter writer, object value)
        {
            SerializeValue(writer, value, false);
        }

		private void SerializeValue(JsonWriter writer, object value, bool isIgnoreCollectionProperty)
		{
			JsonConverter converter;
			...
			{
				converter.WriteJson(writer, value, isIgnoreCollectionProperty);
			}
			else if (value is IConvertible)
			{
				...
				switch (convertible.GetTypeCode())
				{
					...
					case TypeCode.Decimal:
						...
						break;
					default:
                        SerializeObject(writer, value, isIgnoreCollectionProperty);
						break;
				}
			}
			else if (value is IList)
			{
				SerializeList(writer, (IList)value, isIgnoreCollectionProperty);
			}
			else if (value is IDictionary)
			{
                SerializeDictionary(writer, (IDictionary)value, isIgnoreCollectionProperty);
			}
			else if (value is ICollection)
			{
                SerializeCollection(writer, (ICollection)value, isIgnoreCollectionProperty);
			}
		...
		private void WriteMemberInfoProperty(JsonWriter writer, object value, MemberInfo member, bool isIgnoreCollectionProperty)
		{
			if (!ReflectionUtils.IsIndexedProperty(member))
			{
                ...
                //Modify By Zf 2009-7-28
                //Old Start
                //SerializeValue(writer, memberValue);
                //Old End
                //New Start
                if (isIgnoreCollectionProperty && (memberValue is IList || memberValue is IDictionary || memberValue is ICollection))
                {
                    SerializeValue(writer, null, isIgnoreCollectionProperty);
                }
                else
                {
                    SerializeValue(writer, memberValue, isIgnoreCollectionProperty);
                }
                //New End
			}
		}

        private void SerializeObject(JsonWriter writer, object value, bool isIgnoreCollectionProperty)
		{
			...
			foreach (MemberInfo member in members)
			{
				if (ReflectionUtils.CanReadMemberValue(member) && !member.IsDefined(typeof(JsonIgnoreAttribute), true))
                    WriteMemberInfoProperty(writer, value, member, isIgnoreCollectionProperty);
			}
        ...
        private void SerializeCollection(JsonWriter writer, ICollection values, bool isIgnoreCollectionProperty)
		{
			object[] collectionValues = new object[values.Count];
			values.CopyTo(collectionValues, 0);

            SerializeList(writer, collectionValues, isIgnoreCollectionProperty);
		}

        private void SerializeList(JsonWriter writer, IList values, bool isIgnoreCollectionProperty)
		{
			writer.WriteStartArray();

			for (int i = 0; i < values.Count; i++)
			{
                SerializeValue(writer, values[i], isIgnoreCollectionProperty);
			}

			writer.WriteEndArray();
		}

        private void SerializeDictionary(JsonWriter writer, IDictionary values, bool isIgnoreCollectionProperty)
		{
			writer.WriteStartObject();
			...
			foreach (DictionaryEntry entry in values)
			{
                ...
                SerializeValue(writer, entry.Value, isIgnoreCollectionProperty);
			}
